...
export function CapacityView({
  ...
  const overCommittedCount = capacities.filter((user) => {
    const totalHours = user.allocations.reduce(
      (acc, curr) => acc + (Number(curr.hoursPerDay) || 0),
      0
    )
    return totalHours > standardHoursPerDay
  }).length
  ...
  return (
    <div className={cn('space-y-4', className)}>
      {overCommittedCount > 0 && (
        <FxAlert
          tone="destructive"
          className="border-destructive/30 bg-destructive-subtle flex items-center"
        >
          ...
          <span>
            ...
            committed beyond a standard working day.
          </span>
        </FxAlert>
      )}

      <div className="bg-card border-border/70 overflow-hidden rounded-xl border shadow-xs">
        {/* Header */}
            ...
            return (
              <div key={user.userId} className="space-y-3 p-6">
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  <span
                    className={cn('text-xs font-semibold', status.textColor)}
                  >
                    {status.label}
                  </span>
