...
--
-- ALSO DECLARED in schemas/storage/storage.sql, which is registered in
-- config.toml `schema_paths`. That declaration is what stops a regenerated
-- baseline from dropping these four (the `storage` schema is in pg_dump's
-- exclude list) and what stops `db diff` reading them as drift and emitting
-- DROP POLICY. Keep the two in sync; the bucket insert above stays here only
-- because a schema diff never carries rows.
...
-- INSERT: only staff (primary_admin/admin/manager/contributor) can upload
