-- =====================================================================
-- foxy-hub — demo seed data (consolidated)
--
-- SUPERSEDES BOTH of the previous seed migrations. Do not run them alongside
-- this file:
--
--   20260811124449_seed_data.sql       -- first version
--   20260903041730_seed_demo_data.sql  -- reissue of the above, +Sofia/Lumen
--
-- Those two are byte-for-byte identical in the UUIDs and email addresses they
-- insert, and neither carries an `on conflict` guard, so they could only ever
-- both run because the second one DELETES the first one's rows before
-- re-inserting them (section 0 below). Two files doing one file's work.
--
-- This file is the second one's content — it is the newer and strictly better
-- of the two: it adds Sofia Marchetti as a second client login, and it replaces
-- the original's `alter table auth.users disable trigger` block with a check on
-- `handle_new_user_signup`'s own `seed_user` escape hatch. That matters because
-- disabling a trigger on `auth.users` needs ownership of a table owned by
-- `supabase_auth_admin`, which the seed does not have; the escape hatch needs
-- no privileges at all.
--
-- ROLES: uses the renamed enum values, so it MUST sort after
-- 20260922150000_rename_user_role_values.sql. `memberships.role` and
-- `invitations.role` now read 'primary_admin' and 'contributor'.
--
-- The four 'member' values still in section 11 are `activity_events.actor_kind`
-- — a DIFFERENT enum ('system' | 'client' | 'member') that was not renamed and
-- has no 'manager' value, because the feed tints all staff the same. Do not
-- "fix" them.
--
-- Section 0's deletes make the file re-runnable and are harmless no-ops on a
-- fresh database.
--
-- One fully-populated workspace ("Foxy Studio"): five accounts, six projects,
-- and enough rows behind them that every reader in `src/lib/dal.ts` returns
-- something real instead of an empty state.
--
-- THERE IS NO `begin;` / `commit;` HERE. The Supabase CLI wraps each migration
-- in its own transaction; a nested `commit` would close it early and leave the
-- migration-tracking insert outside it. Run standalone with:
--
--   psql "$(supabase status -o env | grep DB_URL | cut -d= -f2)" \
--     -v ON_ERROR_STOP=1 --single-transaction -f <this file>
--
-- `plans` is NOT seeded here — that is 20260729102721_seed_plans.sql.
